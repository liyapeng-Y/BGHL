...
def load_args():
    parser = argparse.ArgumentParser(
        description='Navigating Hierarchy: Hyperbolic Learning on Brain Graphs for Disorder Diagnosis',
        formatter_class=argparse.ArgumentDefaultsHelpFormatter)
    parser.add_argument('--seed', type=int, default=42,
                        help='random seed')
    parser.add_argument('--root_dir', type=str, default="./data/ABIDE_pcp/cpac/", help='root path')
    ...
    parser.add_argument('--outdir', type=str, default='./model/logs',
                        help='output path')
    ...
    parser.add_argument('--curv_init', type=float, default=2.3, help='')  
    parser.add_argument('--entail_weight', type=float, default=0.3, help='')  
    parser.add_argument('--entail_weight_1', type=float, default=0.1, help='')
    parser.add_argument('--weight_att', type=float, default=0.2, help='')
...
def train_epoch(model, loader, criterion, optimizer, lr_scheduler, epoch, use_cuda=False):
    ...
    for i, data in enumerate(loader):
        ...
        optimizer.zero_grad()
        output,L_hier = model(data,train=True)
        loss = criterion(output, data.y) + L_hier
...
def eval_epoch(model, loader, criterion, use_cuda=False, split='Val'):
    ...
    metric = precision_recall_fscore_support(
        labels, result, average='macro', zero_division=0)
...
def main():
    ...
    args_dict = vars(args)
    explicit_params = [
    'd_model', 'dim_feedforward', 'dropout', 'num_heads', 'num_layers',
    'batch_norm', 'pe', 'pe_dim', 'gnn_type', 'use_edge_attr', 
    'edge_dim', 'se']
    cleaned_args_dict = args_dict.copy() 
    for param_name in explicit_params:
        if param_name in cleaned_args_dict:
            del cleaned_args_dict[param_name]
    ...
    dataset = ABIDEDataset(path,'filt_noglobal/raw')     
    ...
    num_workers=0        
        ...
        model = HLBG(in_size=dataset.num_node_features,
                                 num_class=2,
    ...
    results = ["{:.2f}".format(np.mean(acc_list) * 100),"{:.2f}".format(np.std(acc_list) * 100),
               "{:.2f}".format(np.mean(auc_list) * 100),"{:.2f}".format(np.std(auc_list) * 100),
               "{:.2f}".format(np.mean(sen_list) * 100),"{:.2f}".format(np.std(sen_list) * 100),
               "{:.2f}".format(np.mean(spec_list) * 100),"{:.2f}".format(np.std(spec_list) * 100)]
    
    print(acc_list)
    print("test acc mean {:.2f} std {:.2f}".format(np.mean(acc_list)* 100, np.std(acc_list)* 100))
    print("test auc mean {:.2f} std {:.2f}".format(np.mean(auc_list) * 100, np.std(auc_list) * 100))
    print("test sensitivity mean {:.2f} std {:.2f}".format(np.mean(sen_list) * 100, np.std(sen_list) * 100))
    print("test specficity mean {:.2f} std {:.2f}".format(np.mean(spec_list) * 100, np.std(spec_list) * 100))
    print("test recall mean {:.2f} std {:.2f}".format(np.mean(rec_list) * 100, np.std(rec_list) * 100))
    print("test precision mean {:.2f} std {:.2f}".format(np.mean(pre_list) * 100, np.std(pre_list) * 100))
    print("test f1_micro mean {:.2f} std {:.2f}".format(np.mean(f1_list) * 100, np.std(f1_list) * 100))
