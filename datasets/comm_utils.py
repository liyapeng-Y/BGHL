...
def get_comm_index():
    with open('./datasets/node_clus_map.pickle', 'rb') as handle:
        node_clus_map = pickle.load(handle)
    return list(node_clus_map.keys())


def get_dist_matrix():
    result = pyreadr.read_r('./datasets/craddock200.rda')
...
def reindex_edges_and_attrs(edge_indices, edge_attributes, comm_index):
    new_edge_indices = []
    new_edge_attrs = []
    for i, ei in enumerate(edge_indices):
        start = comm_index[i]
        ei_reindexed = ei - start
        new_edge_indices.append(ei_reindexed)
        new_edge_attrs.append(edge_attributes[i])
    return new_edge_indices, new_edge_attrs
...
def get_comm_edge_index(comm_index, edge_index, edge_attr, num_nodes, pos_enc_dim):
    ...
    for start, end in zip(comm_index, comm_index[1:]):
        ...
        deg.append(sub_deg)
        new_edge_indices, new_edge_attrs = reindex_edges_and_attrs(edge_indices, edge_attributes, comm_index)
    return torch.cat(edge_indices, dim=1), torch.cat(edge_attributes, dim=0), torch.cat(pe, dim=0), torch.cat(deg, dim=0), new_edge_indices[0], new_edge_indices[1], new_edge_indices[2], new_edge_indices[3], new_edge_indices[4], new_edge_indices[5], new_edge_indices[6], new_edge_indices[7], new_edge_attrs[0], new_edge_attrs[1], new_edge_attrs[2], new_edge_attrs[3], new_edge_attrs[4], new_edge_attrs[5], new_edge_attrs[6], new_edge_attrs[7]
