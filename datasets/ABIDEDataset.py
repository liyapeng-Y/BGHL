...
from datasets.read_data import read_comm_data
from tqdm import tqdm
from datasets.comm_utils import get_dist_matrix,get_comm_index,get_pheno_info
...
class ABIDEDataset(InMemoryDataset):
    def __init__(self, root, raw_name, transform=None, pre_transform=None):
        ...
        labels = self.data.y
        total = len(labels)
        num_0 = torch.sum(labels == 0).item()
        num_1 = torch.sum(labels == 1).item()
